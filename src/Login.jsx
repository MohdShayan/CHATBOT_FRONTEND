  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://chatbot-backend-livid-eight.vercel.app/api/user/login",
        formData,
    ...
    <div className="min-h-screen flex items-center justify-center bg-black relative">
      ...
      <div className="relative z-20 w-full max-w-md bg-white/5 backdrop-blur-md border border-white/20 rounded-2xl p-8 shadow-lg">
        <h2 className="text-3xl font-bold text-center text-white mb-6">
          Log In to <span className="text-transparent bg-gradient-to-r from-yellow-200 to-green-400 bg-clip-text">GeekBot</span>
        </h2>
